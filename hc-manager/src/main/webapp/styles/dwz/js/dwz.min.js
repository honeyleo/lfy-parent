...
if(alertMsg){
	if (xhr.status==DWZ.statusCode.forbidden){
		 var obj = jQuery.parseJSON(xhr.responseText);
	     alertMsg.error(obj.message || DWZ.msg("forbidden"), {okCall:function(){
	        if ($.pdialog) $.pdialog.checkTimeout();
	        if (navTab) navTab.checkTimeout();
	        if(parseInt(obj.code) == -300){
	        	// 关闭当前Tab
	        	navTab.closeCurrentTab();
	        }
	    }});
	}else if(xhr.status==DWZ.statusCode.error){
